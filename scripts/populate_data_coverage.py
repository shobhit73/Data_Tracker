...
insert into client_data_coverage
  (dsp_short_code, fein, company_name, total_employees, active_employees,
   total_with_payment_method, total_with_emergency_contact,
   active_with_payment_method, active_with_emergency_contact, checked_date)
...
def main():
    ...
    cur.execute(
        ...
        "sum(active_with_emergency_contact) from client_data_coverage")
    a, p, e = cur.fetchone()
    if a:
    ...
    if no_payment:
        print(f"\nZERO payment methods despite active staff ({len(no_payment)}):")
        for name, n in sorted(no_payment, key=lambda x: -x[1]):
            print(f"   {str(name)[:44]:44s} {n:5d} active")
    if no_emergency:
        print(f"\nZERO emergency contacts despite active staff ({len(no_emergency)}):")
        for name, n in sorted(no_emergency, key=lambda x: -x[1]):
            print(f"   {str(name)[:44]:44s} {n:5d} active")
